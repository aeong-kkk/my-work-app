# 샘플발주 내역 리스트화 — 자동화 흐름 (입력 → 처리 → 출력)

> 입력: 앱 화면에 드래그 앤 드롭으로 넣는 첨부 엑셀 파일 (한 번에 여러 개 가능).
> 참고 파일: `inputs/New-2.xlsx` (시트: Info 1개뿐) — 리드타임 계산식(요청일-발주일)은 그대로, Packing 시트가 없어지고 Info 시트 안 "Shipping Information" 구역(I·M열)으로 통합됨. 층구성 위치도 G21→G19로 이동.
...
| 항목 | 시트 | 셀 | 비고 |
|---|---|---|---|
| 발주일 | Info | G1 | 파일 안에 직접 기입됨 |
| 구분(New/Repeat) | Info | F3 | "NEW／Repeat" |
| 모델명 | Info | F5 | Customer Model Name |
| 내부명 | Info | F6 | Internal Model Name |
| Total-T | Info | G18 | 板厚 (F18 라벨 옆 값) |
| 층구성 | Info | G19 | 层构成 (F19 라벨 옆 값) |
| 수량 | Info | I3 ~ I(마지막 행) | "Shipping Information" 구역. 행마다 값, 행 개수만큼 각각 별도 결과로 처리 |
| 요청일 | Info | M3 ~ M(마지막 행) | "Shipping Information" 구역. 행마다 값 |
...
5. 첨부파일에서 "Info" 시트 열기
6. G1 셀 값 읽기 → 발주일
7. F3 셀 값 읽기 → 구분(New/Repeat)
8. F5 셀 값 읽기 → 모델명 (Customer Model Name)
9. F6 셀 값 읽기 → 내부명 (Internal Model Name)
10. G18 셀 값 읽기 → Total-T (板厚)
11. G19 셀 값 읽기 → 층구성
12. "Shipping Information" 구역의 I열(3행부터) 값이 입력된 행을 전부 확인 — 값이 하나도 없으면 에러
13. 확인된 행마다 I열 값 읽기 → 수량
14. 확인된 행마다 M열 값 읽기 → 요청일
...
엑셀 파일 드래그 앤 드롭 입력(여러 개 가능) → 파일 하나씩 처리 → 시트 구조 확인 → (구조 다름) 에러 알림·건너뜀 → (구조 정상) Info 시트 G1 읽기(발주일) → F3 읽기(구분) → F5 읽기(모델명) → F6 읽기(내부명) → G19 읽기(층구성) → Shipping Information 구역 I열 행 수 확인 (값 없으면 에러)
  → 확인된 행마다 I열 읽기(수량) → M열 읽기(요청일) → 행 개수만큼 발주 건 생성
...
flowchart LR
    ...
    D -- 예 --> C[3. Info 시트 G1 읽기: 발주일]
    C --> C2[4. Info 시트 F3 읽기: 구분]
    C2 --> C3[5. Info 시트 F5 읽기: 모델명]
    C3 --> E[6. Info 시트 F6 읽기: 내부명]
    E --> F[7. Info 시트 G19 읽기: 층구성]
    F --> H{8. Shipping Information I열에 값 있는 행 확인}
    H -- 없음 --> H1[에러 알림 후 건너뜀]
    H -- 있음 --> J1[8. 행마다 I열: 수량]
    J1 --> J2[9. 행마다 M열: 요청일]
