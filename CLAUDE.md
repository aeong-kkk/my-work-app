...
- 기능 1 실행 (발주 엑셀 → 리드타임 계산): `python feature1_extract_order.py [엑셀파일경로]` — 경로를 안 주면 기본값으로 `inputs/order sheet.xlsx`를 읽음
...
**기능 1은 엑셀 템플릿의 셀 위치를 하드코딩해서 읽습니다** (Info 시트 1개뿐 — 예전엔 별도 Packing 시트가 있었지만 지금은 Info 시트 안 "Shipping Information" 구역으로 통합됨): `I1`=발주일, `G3`=구분(New/Repeat), `G5`=모델명(Customer Model Name), `G6`=내부명(Internal Model Name), `G26`=Total-T(板厚), 층구성(层构成)은 `G27`(층수, 예: "10L")과 `I27`(나머지 스펙, 예: "FVSS3")로 나뉘어 있어 "10L/FVSS3" 형식으로 합쳐서 다룸, 수량·요청일은 항상 "Shipping Information" 구역의 `D`열(수량)/`H`열(요청일), 15행부터(단일 발주든 분할 발주든 구분 없이 이 구역 하나로 처리 — 행이 1개면 단일, 여러 개면 분할). 리드타임은 `요청일 - 발주일`로 계산합니다. 행이 여러 개인데 요청일이 서로 다르면, 그중 가장 늦은 요청일인 행만 구분 값을 "분할입고"로 덮어씁니다(원래 신규발주/재발주였는지와 무관). 원본 엑셀 양식이 바뀌면 `feature1_extract_order.py`의 셀 상수(`SHIPPING_INFO_START_ROW`/`SHIPPING_QTY_COL`/`SHIPPING_DATE_COL` 및 `ORDER_DATE_CELL`/`ORDER_TYPE_CELL`/`MODEL_NAME_CELL`/`INTERNAL_NAME_CELL`/`TOTAL_T_CELL`/`LAYER_COUNT_CELL`/`LAYER_SPEC_CELL`)와 `specs/sample-order-listing-flow.md`의 매핑 표를 같이 업데이트해야 합니다. `inputs/order sheet.xlsx`가 현재 기준이 되는 참고 양식(파일)입니다(이전 기준이던 `New-2.xlsx`와 그 이전 버전들은 셀 위치가 이번에 전면 변경되면서 삭제함). 이 저장소 밖에 이보다 오래된 양식 파일이 남아있을 수 있는데, 그건 템플릿으로 쓰면 안 됩니다.
